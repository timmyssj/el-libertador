...
public:
		Enemigo(float x, float y, SanMartin* h) : Personaje(x, y, 80, 5) {
			heroe = h; cooldownAtaque = 0; cargarTextura();
		}
		...
		void cargarTextura() override {
			...
			if (!t.cargadas) {
				...
				cargarSecuencia(t.animAbajo, "realista_frente", 1);
				cargarSecuencia(t.atkDerecha, "realista_ataque_derecho", 1);
				cargarSecuencia(t.atkIzquierda, "realista_ataque_izquierdo", 1);
				cargarSecuencia(t.atkArriba, "realista_ataque_atras", 1);
				cargarSecuencia(t.atkAbajo, "realista_ataque_frente", 1);
				t.cargadas = true;
			}
		...
		void actualizar() override {
			...
			float mejorPuntaje = 9999.0f; float distFisicaAlMejor = 9999.0f;
